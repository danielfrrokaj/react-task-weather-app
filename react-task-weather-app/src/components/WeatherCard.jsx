...
const WeatherCard = ({ selectedCountry, initialCity, searchResults, isEmbedded = false }) => {
    ...
    return (
        <div className={`p-6 ${isEmbedded ? '' : 'max-w-4xl mx-auto'}`}>
            {/* Only show weather details if not embedded */}
                    ...
                    <div className="relative h-[400px] rounded-xl overflow-hidden shadow-xl animate-scaleIn">
                        ...
                        <div className="absolute bottom-0 w-full p-6 text-white">
                            <div className="flex justify-between items-end">
                                <div>
                                    <h2 
                                        className={`text-4xl font-bold mb-2 text-white ${isAnimating ? 'animate-slideInLeft' : ''}`}
                                    >
                                        {currentCity}
                                    </h2>
            ...
            {showRecommendations && (
                <div className={`backdrop-blur rounded-lg shadow-lg p-6 ${isEmbedded ? 'bg-transparent' : 'bg-slate-800/50'}`}>
                    <AIRecommendations 
